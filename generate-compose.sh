...
if [ "$#" -lt 2 ]; then
    echo "Uso: $0 <archivo_salida> <workers_twentieth_century> [<workers_main_movie> <workers_esp_production> <workers_top5> <workers_sentiment> <workers_arg_production>]"
    exit 1
fi
...
python3 docker-compose-generator.py "$output_file" "$file" "$workers_twentieth_century" "$workers_main_movie" "$workers_esp_production" "$workers_top5" "$workers_sentiment" "$workers_arg_production"
