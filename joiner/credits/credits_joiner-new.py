...
class CreditsJoinerSimple(Worker):
    ...
    def _persist_movies_immediately(self, client_id, movies):
        ...
        try:
            ...
            os.replace(temp_file, MOVIES_PERSISTENCE_FILE) # esto es seguro?
            self.logger.info(f"✅ Movies del cliente {client_id} persistidas inmediatamente ({len(movies)} películas)")
            # TODO aca ya se podria mandar el ack al consumer de movies
    ...
    def _restore_state(self, checkpoint_data):
        ...
        # Verificar consistencia entre checkpoint y persistencia # TODO porque hace falta chequear? en que caso puede fallar?
    ...
    def start(self):
        ...
        try:
            ...
            if len(self.movies.keys()) > 0:
                ...
                self._start_credits_consumer_with_retry()
            else:
    ...
    def _reprocess_pending_messages_after_recovery(self):
        # caso de cuando tengo >1 clientes y me llegan credits de uno al que no le llego movies
    ...
    def _start_credits_consumer_with_retry(self):
        ...
        # TODO porque tenemos que reintentar?
